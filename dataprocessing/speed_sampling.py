...
def arm_rand_sample_bound_points(numsamples, dim, 
    ...
    while OutsideSize > 0:
        ...
        for batch_id in range(math.floor(x0.shape[0]/batch_size)+1):
            ...
            query_points = 0.4*query_points.cpu().numpy()#.unsqueeze(dim=0)
            ...
            distance, closest_faces, closest_points = igl.point_mesh_squared_distance(query_points, vertices, faces)

            
            distance = torch.sqrt(torch.from_numpy(distance).cuda()).squeeze()
...
def point_rand_sample_bound_points(numsamples, dim, 
    ...
    while OutsideSize > 0:
        ...
        query_points = x0
        query_points = query_points.cpu().numpy()#.unsqueeze(dim=0)
        ...
        unsigned_distance = torch.sqrt(torch.from_numpy(distance).cuda()).squeeze()
        ...
        y1 = torch.sqrt(torch.from_numpy(distance).cuda()).squeeze()
