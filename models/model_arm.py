...
class NN(torch.nn.Module):
    ...
    def __init__(self, device, dim, out_path_):#10
        ...
        with open(out_path_+'/dim') as f:
            ...
            for line in f:
                ...
                if iter==0:
                    ...
                    print(end_effect_)
                iter=iter+1
        path_name_ = os.path.splitext(os.path.basename(out_path_))[0]
        ...
        tg_batch = self.chain.forward_kinematics(torch.zeros(1,self.dim).to(device), end_only = False)
        ...
        for tg in tg_batch:
            ...
            if iter>1:
                ...
                self.v_list.append(torch.from_numpy(v).float().cuda())
    ...
    def FK(self,theta):
        scale=1.8*math.pi
        theta = scale*theta
        tg_batch = self.chain.forward_kinematics(theta, end_only = False)
...
class Model():
    ...
    def train(self):
        ...
        self.network = NN(self.Params['Device'],self.dim,self.Params['DataPath'])
        ...
        for epoch in range(1, self.Params['Training']['Number of Epochs']+1):
            ...
            self.optimizer.param_groups[0]['lr']  = max(1e-4*(1-epoch/self.l0),1e-5)
            ...
            while True:
                ...
                if (diff_ratio < 2.0 and diff_ratio > 0):#1.5
                    break
                else:
